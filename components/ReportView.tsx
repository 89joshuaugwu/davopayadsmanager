...
export default function ReportView({ gmailAccounts, businessCenters, adsAccounts, totals, rangeLabel }: Props) {
  ...
  return (
    <div id="report-print-area" className="bg-white border border-davo-border rounded-xl2 shadow-card print-page p-5 sm:p-8">
      <div className="flex items-center justify-between pb-5 border-b border-davo-border mb-6">
      ...
      <div className="grid grid-cols-4 gap-3 mb-8">
        {[
          ...
          <div key={c.label} className="border border-davo-border rounded-xl p-3">
            <p className="text-[11px] text-davo-muted uppercase tracking-wide mb-1">{c.label}</p>
            <p className="text-sm sm:text-base font-bold text-davo-navy">{formatCurrency(c.value)}</p>
          </div>
        ))}
      </div>

      <h3 className="text-sm font-semibold text-davo-navy mb-3">Business Centers</h3>
      <table className="w-full text-xs sm:text-sm mb-8 border-collapse">
        <thead>
